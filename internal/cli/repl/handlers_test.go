...
func TestContextStatus_UpdatesOnUsageEvent(t *testing.T) {
	...
	m.agentCore = newAgentCore(nil, t.TempDir())
	...
	updatedAfterUsage, _ := updatedAfterChunk.handleLLMUsage(&agentcore.TokenUsage{InputTokens: 1000})
...
func TestHandleLLMDoneDrainsSubagentActivity(t *testing.T) {
	...
	activity <- agentcore.ToolActivity{RunID: "run-1", CallID: "tool-1", Agent: "worker", Event: agentcore.StreamEvent{
		Type:     agentcore.StreamEventTypeToolStart,
		ToolCall: &agentcore.ToolCall{Name: "bash", Input: map[string]any{"command": "go test ./..."}},
	}}
	activity <- agentcore.ToolActivity{RunID: "run-1", CallID: "tool-1", Agent: "worker", Event: agentcore.StreamEvent{
		Type:     agentcore.StreamEventTypeToolEnd,
		ToolCall: &agentcore.ToolCall{Name: "bash"},
	}}
	m := replModel{
		stream:           streamState{handler: sh},
		loading:          loadingState{showSpinner: true},
		width:            80,
		agentCore:        newAgentCore(nil, t.TempDir()),
...
func TestHandleLLMDone(t *testing.T) {
	...
	if len(m.agentCore.GetMessages()) != 1 {
		t.Errorf("expected 1 message in history, got %d", len(m.agentCore.GetMessages()))
	}
	if m.agentCore.GetMessages()[0].Role != agentcore.RoleAssistant {
		t.Errorf("expected assistant role, got %s", m.agentCore.GetMessages()[0].Role)
	}
	if m.agentCore.GetMessages()[0].Content != "response line 1\nresponse line 2" {
		t.Errorf("unexpected message content: %s", m.agentCore.GetMessages()[0].Content)
	}
...
func TestHandleKeyMsg_CtrlC_WithActiveStreamDrainsNextQueuedInput(t *testing.T) {
	...
	m.agentCore = newAgentCore(client, "", m.ctx.cfg)
	m.stream.handler.Start(make(chan agentcore.StreamEvent), "Loading...")
	m.loading.showSpinner = true
	m.stream.cancel = func() {}
	...
	if messages := newM.agentCore.GetMessages(); len(messages) != 1 || messages[0].Content != "next prompt" {
...
func TestHandleLLMStreamMsg_IgnoresPreviousStreamAfterQueuedInputStarts(t *testing.T) {
	m := newTestModel()
	oldEventCh := make(chan agentcore.StreamEvent)
	...
	newM, cmd, handled := m.handleLLMStreamMsg(mainStreamMsg{
		eventCh: oldEventCh,
		event:   agentcore.StreamEvent{Type: agentcore.StreamEventTypeError, Error: context.Canceled},
	})
...
func TestHandleKeyMsg_ShiftTabTogglesMode(t *testing.T) {
	...
	if newM.agentCore.Mode() != agentcore.ModeYolo {
		t.Fatalf("expected app state yolo mode, got %q", newM.agentCore.Mode())
	}
	...
	if newM.agentCore.Mode() != agentcore.ModeBuild {
		t.Fatalf("expected app state build mode, got %q", newM.agentCore.Mode())
	}
}
...
func TestHandleLLMDone_EmptyResponse(t *testing.T) {
	...
	if len(m.agentCore.GetMessages()) != 1 {
		t.Errorf("expected 1 message, got %d", len(m.agentCore.GetMessages()))
	}
...
func TestHandleLLMError_MaterializesMessageAndTurnMemory(t *testing.T) {
	...
	m := replModel{
		...
		loading:   loadingState{showSpinner: true},
		width:     80,
		agentCore: newAgentCore(nil, workingDir),
		output:    reploutput.NewOutputBuilder(80, ""),
	}
	...
	messages := updated.agentCore.GetMessages()
	if len(messages) != 1 {
...
func TestAutoCompactionAppliedStripsSystemMessagesFromAppState(t *testing.T) {
	...
	updated, _ := m.handleAutoCompactionApplied(&agentcore.AutoCompactionEvent{Replacement: []agentcore.Message{
		{Role: agentcore.RoleSystem, Content: "provider system prompt"},
		{Role: agentcore.RoleUser, Content: "checkpoint"},
	}})

	messages := updated.agentCore.GetMessages()
	if len(messages) != 1 || messages[0].Role != agentcore.RoleUser || messages[0].Content != "checkpoint" {
...
func TestAutoCompactionAppliedReplacesHistoryWithoutAppendingCheckpoint(t *testing.T) {
	...
	m.agentCore.AppendMessage(agentcore.Message{Role: agentcore.RoleUser, Content: "original task"})
	m.compaction.active = true
	m.compaction.mode = compactionAutomatic
	replacement := []agentcore.Message{{Role: agentcore.RoleUser, Content: "checkpoint"}}

	updated, _ := m.handleAutoCompactionApplied(&agentcore.AutoCompactionEvent{Replacement: replacement})
...
func TestHandleLLMIncompletePreservesPartialResponse(t *testing.T) {
	...
	if messages := updated.agentCore.GetMessages(); len(messages) != 0 {
...
func TestAutomaticCompactionLifecycle(t *testing.T) {
	...
	started, cmd := m.handleAutoCompactionStarted(&agentcore.AutoCompactionEvent{Cancel: func() { cancelled = true }})
...
func TestHandleLLMStreamMsgRoutesMainEvents(t *testing.T) {
	tests := []struct {
		...
		{name: "chunk", event: agentcore.StreamEvent{Type: agentcore.StreamEventTypeChunk, Content: "answer"}, check: func(t *testing.T, m replModel) {
			if m.stream.handler.GetResponse() != "answer" {
				t.Fatal("chunk was not routed")
			}
		}},
		{name: "reasoning", event: agentcore.StreamEvent{Type: agentcore.StreamEventTypeReasoningChunk, Content: "thought"}, check: func(t *testing.T, m replModel) {
			if !m.stream.handler.HasContent() {
				t.Fatal("reasoning was not routed")
			}
		}},
		{name: "usage", event: agentcore.StreamEvent{Type: agentcore.StreamEventTypeUsage, Usage: &agentcore.TokenUsage{InputTokens: 7}}, check: func(t *testing.T, m replModel) {
			if m.agentCore.GetLastUsage() == nil || m.agentCore.GetLastUsage().InputTokens != 7 {
				t.Fatal("usage was not routed")
			}
		}},
		{name: "retry", event: agentcore.StreamEvent{Type: agentcore.StreamEventTypeRetry, Attempt: 2}, check: func(t *testing.T, m replModel) {
